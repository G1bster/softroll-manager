...
--                            Лише додає/оновлює — клієнт НІКОЛИ не стирає
--                            локальні резерви гравців, яких хост не згадав
--                            у цьому раунді (раніше стирав; небезпечно, якщо
--                            це НОВИЙ хост зі своїми ще неповними даними,
--                            напр. одразу після дисконекту попереднього РЛ).
...
-- Локальні резерви НЕ стираються — ані заздалегідь, ані після Z: відповідь
-- (Y) лише додає/оновлює те, що надіслав хост (див. OnSyncPlayer), а
-- OnSyncComplete більше нічого не прибирає локально. Раніше тут була
-- diff-синхронізація, що видаляла кожного гравця, якого хост жодного разу
-- не згадав у раунді — небезпечно, якщо hostName щойно змінився (новий
-- хост після дисконекту/зміни лідера) і в нього самого ще неповні дані:
-- це стирало б правильні локальні резерви решти рейду.
...
--- Завершення раунду синхронізації (Q…Z). НАВМИСНО лише додає/оновлює дані
-- з "Y" (див. OnSyncPlayer) і нічого не видаляє тут — раніше тут прибирався
-- кожен локальний гравець, якого хост жодного разу не згадав у цьому раунді.
-- Це виглядало розумно як diff-синхронізація для звичайного "хост той
-- самий, дані просто застаріли" випадку, але стає катастрофою, якщо
-- SENDER — це НОВИЙ хост (інша людина щойно стала РЛ/отримала сесію,
-- напр. після дисконекту попереднього): OnSessionStart просить повний
-- ресинк саме тоді, коли self.sessionHost змінюється, а в нового хоста
-- db.reserves майже напевно неповний або порожній — і цей блок стирав би
-- ПРАВИЛЬНІ локальні дані решти рейду, підганяючи їх під порожню копію
-- нового хоста. Явне видалення одного гравця (порожній itemStr у "Y" від
-- BroadcastPlayerSync) чи повне "W" лишаються єдиними шляхами стирання.
